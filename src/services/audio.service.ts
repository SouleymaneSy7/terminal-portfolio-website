...
class AudioService {
  ...
  private playClick(ctx: AudioContext, freq: number, duration: number, type: OscillatorType): void {
  ...
  private playChime(ctx: AudioContext, freqs: number[], duration: number): void {
    freqs.forEach((freq, i) => {
      ...
      gainNode.gain.linearRampToValueAtTime(g, ctx.currentTime + i * 0.06 + 0.01);
      gainNode.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + i * 0.06 + duration);
